...
test.describe('Dashboard', () => {
  ...
  test('clicking a target opens the create-session modal and launches', async ({ page }) => {
    // Click the first target card — should open the confirmation modal
    // instead of navigating immediately, so users get a chance to pick
    // Solo vs Collaborative before the PTY spawns.
    await page.locator('.target-card').first().click();

    const dialog = page.getByRole('dialog', { name: 'Start a session' });
    await expect(dialog).toBeVisible();

    // Default mode is Collaborative (multiplexed). Cancel + re-open to
    // verify the modal state resets on each open, then launch for real.
