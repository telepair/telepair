...
Playback reads `.cast` files directly from `/api/recordings/{id}/data`, either via bearer token (owner / admin) or via an `X-Share-Token: <raw>` request header (anonymous). The `/recordings/{id}/play` route sits outside `AuthGuard` so anonymous viewers with a share link (shape: `…/play#token=<raw>`) are not bounced to `/login`; the player reads the URL fragment, scrubs it from history via `replaceState`, then issues the single data fetch with the header. Keeping the secret out of the query string is deliberate — reverse-proxy access logs (`nginx $request`, ALB `request_url`, CloudFront standard logs) capture query strings by default but never custom request headers, so a leaked log file cannot replay a still-valid share link.
...
- **Admin approval gate**: New email-registered users are inserted with `approval_state = 'pending'` and `session_enabled = FALSE`. An admin approves via `POST /api/admin/users/{id}/enable`, which flips `session_enabled = TRUE` and sets `approval_state = 'approved'` in a single transaction. `POST /api/admin/users/{id}/disable` is the reverse — it flips `session_enabled = FALSE` but leaves `approval_state = 'approved'`, so "admin disabled an active user" and "still waiting for first approval" remain distinguishable in the admin UI and audit trail. Login itself is permitted in both pending and disabled states so the user can view history or change their password while waiting.
