...
- Owners can mint **signed share links** (TTL + max-uses) via `POST /api/recordings/{id}/shares`. Anonymous viewers hitting `/recordings/{id}/play#token=...` bypass `AuthGuard`; the player reads the URL fragment, scrubs it from history via `replaceState`, then fetches `/api/recordings/{id}/data` with an `X-Share-Token: <raw>` header. The token is validated atomically against the recording id, remaining uses, and expiry in a single `UPDATE … RETURNING`. Keeping the secret out of the query string matters for log hygiene — NGINX `$request` / ALB `request_url` / CloudFront standard logs all capture query strings by default, but none of them log `X-Share-Token` unless explicitly told to.
